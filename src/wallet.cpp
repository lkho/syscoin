                           ...
                           list<pair<CTxDestination, int64> >& listSent, int64& nFee, string& strSentAccount, bool &fNameTx) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        if(   !ExtractDestination(txout.scriptPubKey, address)
            ...
            &&!ExtractAssetAddress(txout.scriptPubKey, saddress) ) {
        ...
        else if (IsAliasMine(*this, txout, true)) {
            pushBack = true;
        }
        else if (IsOfferMine(*this, txout, true)) {
            pushBack = true;
        }
        else if (IsCertMine(*this, txout, true)) {
            pushBack = true;
        }
        else if (IsAssetMine(*this, txout, true)) {
            pushBack = true;
        }
        if(pushBack)
...
void CWalletTx::RelayWalletTransaction()
{
    ...
    {
        // TODO CB This sounds exactly like what we've been seeing??
...
// populate vCoins with vector of spendable COutputs
void CWallet::AllSpendableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl) const
{
    vCoins.clear();
...
string CWallet::SendData(CWalletTx& wtxNew, bool fAskFee, const std::string& txData)
{
    ...
    if (!CreateTransaction(scriptPubKey, 0, wtxNew, reservekey, nFeeRequired, strError, NULL, txData))
    {
        if (nFeeRequired > GetBalance())
            strError = strprintf(
            		_("Error: This transaction requires a transaction fee of at least %s"
            		  "because of its amount, complexity, or use of recently received "
            		  "funds!"), FormatMoney(nFeeRequired).c_str());
        printf("SendData() : %s\n", strError.c_str());
        return strError;
    }
