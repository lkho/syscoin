...
public:
			...
			try {
				...
				if (result.type() == array_type)
				{
					...
					BOOST_FOREACH(Value& input, arr)
					{
						...
						const Value& guid_value = find_value(o, "GUID");
						if (guid_value.type() == str_type)
							guid_str = guid_value.get_str();
						const Value& from_value = find_value(o, "from");
						if (from_value.type() == str_type)
							from_str = from_value.get_str();
						const Value& to_value = find_value(o, "to");
						if (to_value.type() == str_type)
						...
						const Value& subject_value = find_value(o, "subject");
						if (subject_value.type() == str_type)
							subject_str = subject_value.get_str();
						const Value& message_value = find_value(o, "message");
						if (message_value.type() == str_type)
    ...
    void updateEntry(const QString &guid, const QString &time, const QString &from, const QString &to, const QString &subject, const QString &message, MessageModelType type,int status)
    {
        ...
        case CT_NEW:
            ...
            if(inModel || index != -1)
            {
                OutputDebugStringF("Warning: MessageTablePriv::updateEntry: Got CT_NOW, but entry is already in model\n");
                break;
        ...
        case CT_UPDATED:
            if(!inModel)
            {
                OutputDebugStringF("Warning: MessageTablePriv::updateEntry: Got CT_UPDATED, but entry is not in model\n");
                break;
            }
            lower->guid = guid;
			lower->time = time;
        ...
        case CT_DELETED:
            if(!inModel)
            {
                OutputDebugStringF("Warning: MessageTablePriv::updateEntry: Got CT_DELETED, but entry is not in model\n");
                break;
            }
